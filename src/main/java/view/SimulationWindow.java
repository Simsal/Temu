...
import model.ConfigurationInformation;
	...
	public class SimulationWindow implements ActionListener {
		...
		JLabel simulationStateLabel, simulationState, actualConfiguration, simulation, deviceNameLabel, deviceName,
				...
				unityMeasurementStart, measurementEndingLabel, measurementEnding, actualMeasurementLabel, actualMeasurement,
				emptyLabel0, emptyLabel1, emptyLabel2, emptyLabel3, emptyLabel4, emptyLabel5, emptyLabel6, emptyLabel7, emptyLabel8,
				emptyLabel9, emptyLabel10 ,emptyLabel11;
		...
		public SimulationWindow(SerialPort chosenPort) {
			...
			window = new JFrame("Simulation von " + connectedPort.getDescriptivePortName());
			window.setLayout(new GridLayout(8, 4));
			
			firstGrid = new JPanel();
			
			simulationStateLabel = new JLabel("<html><b style='float:left'>Simulation: </b></html>");
			firstGrid.add(simulationStateLabel);

			simulationState = new JLabel("<html><p style='color:#04B431'>Nicht aktiv</p></html>");
			...
			ImageIcon image2 = new ImageIcon("src/main/resources/images/questionmark.jpg");
			...
			measurementStartLabel = new JLabel(localModel.getMeasurementValueStart());
			...
			actualMeasurement = new JLabel(communicationInterface.getCurentMeasurementValue(connectedPort));
			window.add(actualMeasurement);

			simulatedMeasurementValue = new JTextField();
			window.add(simulatedMeasurementValue);

			ImageIcon image7 = new ImageIcon("src/main/resources/images/questionmark.jpg");
			JLabel infoLabel7 = new JLabel(image7, JLabel.CENTER);
			infoLabel7.setToolTipText("<html>Links wird der empfangene Messwert angezeigt. <br> "
		...
		public void actionPerformed(ActionEvent e) {
			...
			if ((JButton) e.getSource() == startSimulation) {
				if(simulatedMeasurementValue.getText().isEmpty()){
					JOptionPane.showMessageDialog(new JOptionPane(), "Bitte geben Sie einen Wert zum simulieren an.");
				}
				else {
					try {
						communicationInterface.newSimulation(connectedPort, simulatedMeasurementValue.getText());
						simulationState.setText("<html><p style='color:#F22C2C'>Aktiv</p></html>");
			...
			if ((JButton) e.getSource() == stopSimulation){
				communicationInterface.stopSimulation(connectedPort);
				simulatedMeasurementValue.setEnabled(true);
				startSimulation.setEnabled(true);
				stopSimulation.setEnabled(false);
				simulationState.setText("<html><p style='color:#04B431'>Nicht aktiv</p></html>");
